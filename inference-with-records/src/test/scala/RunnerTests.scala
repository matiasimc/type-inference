...
class RecordTest extends FlatSpec with Matchers {
  val expr1 = GetFromRecord(Record(List(RecPair('bar, Num(4)))), 'bar)
  s"The evaluation of the expression $expr1" should "return ValInt(4)" in {
    assert(Runner.execute(expr1) == ValInt(4))
  }
  val expr2 = Record(List(RecPair('foo, Fun(Id('fun),TNum(),Id('x),Add(Num(4), Id('x)))),
    RecPair('bar,  Num(4))))
  ...
  val expr3 = With(Id('r), TRecord(List(TRecPair('foo, TNum()))), Record(List(RecPair('foo, Num(4)))), Mul(GetFromRecord(Id('r), 'foo), Num(3)))
